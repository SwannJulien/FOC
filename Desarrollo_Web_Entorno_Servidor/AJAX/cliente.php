 ...
 <body>
    <div class="wrapper">
        ...
        <form action="" method="get">
            ...
            <label for="">Búsqueda:<input type="text" id="texto"></label>
        </form>
<?php
if (isset($_GET['reset'])) {
    header("Location: http://localhost/dwes/08/local/src/cliente.php");
}
...
<?php
    if (isset($_GET['consulta_autores'])) {
        $lista_autores = file_get_contents('http://localhost/dwes/08/local/src/api.php?action=get_listado_autores');
...
<?php foreach($lista_autores as $autores): ?>
            <tr>
                ...
                <td><a href="<?php echo "http://localhost/dwes/08/local/src/cliente.php?action=get_datos_autor&id=" . $autores->id  ?>">Detalles</a></td>
            </tr>
<?php endforeach; ?>
        </table>
</div>
...
if (isset($_GET["action"]) && isset($_GET["id"]) && $_GET["action"] == "get_datos_autor") 
{
    ...
    $app_info = file_get_contents('http://localhost/dwes/08/local/src/api.php?action=get_datos_autor&id=' . $_GET["id"]);
...
<div id="hide" class="center">
    ...
    <?php foreach($app_info->libros as $libro): ?>
        <tr>
            ...
            <td><a href="<?php echo "http://localhost/dwes/08/local/src/cliente.php?action=get_datos_libro&id=" . $libro->id?>"><?=$libro->titulo?></a></td>
        </tr>
    <?php endforeach; ?>
        </table>
</div>
...
<?php
    if (isset($_GET['consulta_libros'])) {
        $lista_libros = file_get_contents('http://localhost/dwes/08/local/src/api.php?action=get_listado_libros');
...
<?php foreach($lista_libros as $libros): ?>
            ...
            <td><a href="<?php echo "http://localhost/dwes/08/local/src/cliente.php?action=get_datos_libro&id=" . $libros->id  ?>">Detalles</a></td>
            </tr>
<?php endforeach; ?>
        </table>
</div>
...
{
    //Se realiza la peticion a la api que nos devuelve el JSON con la información de los autores
    $app_info = file_get_contents('http://localhost/dwes/08/local/src/api.php?action=get_datos_libro&id=' . $_GET["id"]);
    //var_dump($app_info);
    // Se decodifica el fichero JSON y se convierte a array
    $app_info = json_decode($app_info);
?>
<div id="hide" class="center">  
        ...
        <!-- Mostramos los libros del autor -->
            <tr>
                ...
                <td><?=$app_info->datos->nombre?></td>
                <td><a href="<?php echo "http://localhost/dwes/08/local/src/cliente.php?action=get_datos_autor&id=" . $app_info->datos->id?>"><?=$app_info->datos->apellidos?></a></td>
            </tr>
        </table>
</div>
<?php

...
function buscar_libro($nombre){
    try {
        $sql = "SELECT * FROM Libro WHERE titulo LIKE '%$nombre%'";
        $lectura = conexion()->query($sql);
...
$posibles_URL = array("get_listado_autores", "get_datos_autor", "get_listado_libros", "get_datos_libro", "buscar_libro");
...
if (isset($_GET["action"]) && in_array($_GET["action"], $posibles_URL))
{
  switch ($_GET["action"]) {
    ...
    case "buscar_libro":
        ...
        $valor = buscar_libro($_GET["nombre"]);
        else
        $valor = "Argumento no encontrado";
        break; 
    }
